...
- [x] 취소 정책: DRAFT/OPEN/RUNNING → CANCELLED, 순위·보상 미생성 + 뛰던 트랙 개인 기록 보존(업로드 수락·집계 제외·히스토리 배지 — track-api v0.1.2) (2026-07-05 M2-C)
...
### 레이스 규칙 구현
- [x] **그로스 타임**: 자동 일시정지 없음 — 정제에서 정지 구간 미삭제 골든으로 박제 (2026-07-04 M2-A)
- [x] **기록 구간 = 시작 버튼 ~ 도착점 반경 최초 진입 시각 자동 확정** — FinishPolicy 서버 판정 (2026-07-04 M2-A)
- [ ] 종료 버튼 = 트래킹 중단 + 업로드 트리거만 (클라 배선 — M2-B)
- [x] 출발은 각자 시작 버튼 기준 (STARTED 신호, 개인 소요 시간 순위) (B2/M2-A)
...
### Tracking 서버 (데이터 심장부)
- [x] TrackRecord 애그리거트 (2026-07-04 M2-A)
- [x] 저장 분리: track_record + track_payload 1:1, 포트 분리로 조회 경로 격리 (QA 5차 JOIN 0 실증) (2026-07-04 M2-A)
- [x] TrackUploadService: client_upload_id 멱등, 병렬 배열 검증, 8MiB 상한(R-006), 크루 경계 403(R-007) — 코스 이탈은 FinishPolicy 일원화 (2026-07-04 M2-A)
- [x] TrackRefinementService (GPS 정제) — 파라미터 외부화(50m/12mps/윈도3/gap30s), 골든 12케이스 (2026-07-04 M2-A):
  - [x] accuracy 임계 초과 포인트 제거
  - [x] 이동 평균 스무딩
  - [x] 이상 점프 보정
  - [x] 총 거리·페이스는 정제 후 좌표로 계산
  - [x] 원시 + 정제 결과 모두 보존
  - [x] GPS 유실 구간 식별 메타
- [x] TrackSegment: 500m 페이스 요약 (유령 구간 R-008 수정·골든 박제) (2026-07-04 M2-A)
- [x] TrackUploaded 이벤트(AFTER_COMMIT) → 전원 체크 → 자동 확정 파이프라인 (2026-07-04 M2-A)

### FinishPolicy (완주 판정) — 순수 함수
- [x] 조건 ①②③ + AND 판정, DNF 기록·경로 보존, finished_at 최초 진입 자동 확정 — 골든 18케이스(임계 직전/직후/등호 전수) (2026-07-04 M2-A)
- [x] 임계값(30m/90%/80%/50m) 설정 외부화 (2026-07-04 M2-A)
- [x] 코스 이탈 검증 FinishPolicy 일원화 (QA 5차 중복 부재 확인) (2026-07-04 M2-A)

### 세션 마감 처리 (Race 컨텍스트 소속 — Participation 상태 변경·RaceCompleted 발행 책임)
- [x] RaceCompleted 트리거: 전원 업로드 또는 upload_deadline 도달 (스케줄러, clock 주입) (2026-07-04 M2-A)
- [x] 마감 시 미처리자 정리: STARTED 미업로드→DNF, REGISTERED→DNS — 골든 박제 (2026-07-04 M2-A)

### Ranking 컨텍스트
- [x] RankingPolicy: 오름차순, 동률 공동 순위+건너뜀(1,1,3) — 골든(3자 동률 1,1,1,4 포함) (2026-07-04 M2-A)
- [x] DNF/DNS 순위 미부여, 하단 표기 (2026-07-04 M2-A)
- [x] RaceResult / RankEntry 확정 + ResultFinalized 이벤트 (rank 백틱 인용 — R-003 이월 종결) (2026-07-04 M2-A)
- [x] PersonalBest: 유저×코스, 완주 기록만(DNF 제외 — O-M2-5 확정) (2026-07-04 M2-A)
- [x] 순위표·기록 히스토리 화면: 내 기록 목록(취소 배지·DNF 표기)·코스별 PB — history-api v0.1 (2026-07-05 M2-C)
- [x] 코스 등록 ② 과거 주행 기록 승격 — 크루 멤버·본인 FINISHED 트랙·1km 하한, refined 서버 재확정 (2026-07-05 M2-C)

### 테스트 (이 마일스톤의 핵심 산출물)
- [x] FinishPolicy 골든 (정상/지름길/다른 길/미진입/경계 등호 — 18케이스) (2026-07-04 M2-A)
- [x] TrackRefinement 골든 (12케이스, 그로스 타임 포함) (2026-07-04 M2-A)
- [x] RankingPolicy 테스트 (동률·DNF/DNS 혼합·PB — 9케이스) (2026-07-04 M2-A)
...
- [x] `ReplayGenerationService` + 전용 소스 포트(RP-1 격리) — 프로젝션 계층 (2026-07-05 M3-A)
- [x] 전원 트랙 t=0 상대 시각 정렬·병합 (2026-07-05 M3-A)
  - [x] 정렬·병합 골든 (DNF 조기종료·is_gap 경계 포함) (2026-07-05)
- [x] 추월 사전 계산 (부호반전 정의, 동시도달 비이벤트·재역전 순차 — 골든) (2026-07-05 M3-A)
- [x] 구간 페이스 색상 버킷 (TrackSegment 재사용, 경계 골든) (2026-07-05 M3-A)
- [x] schema_version(v1, append-only 규약) + GENERATING→READY/FAILED (2026-07-05 M3-A)
- [x] ResultFinalized AFTER_COMMIT + @Async 비동기 생성 (2026-07-05 M3-A)
- [x] FAILED 관측(log.error) + 재생성 경로 (2026-07-05 M3-A)
- [x] 재생성 admin API (멱등 — 동일 바이트 실증) (2026-07-05 M3-A)
- [x] replay_snapshot 저장 (2MiB 상한 → FAILED) (2026-07-05 M3-A)
- [x] 리플레이 조회 REST (status별 응답·display_names 조인·403/404) (2026-07-05 M3-A)
- [x] 리플레이 조회 이벤트 로깅 (viewed_within_24h — M4 측정 가능 확인) (2026-07-05 M3-A)

### 리플레이 뷰어 (클라이언트)
- [x] 마커 보간 애니메이션 (전원 동시 이동, ReplayPlayback 순수 골든) (2026-07-05 M3-B)
- [x] GPS 유실 구간 점선 구분 (is_gap) (2026-07-05 M3-B)
- [x] 배속 재생 (1x/2x/4x) (2026-07-05 M3-B)
- [x] 시간축 슬라이더 (드래그 시킹) (2026-07-05 M3-B)
- [x] 추월 지점 마킹 (지도+타임라인+근접 배너) (2026-07-05 M3-B)
- [x] 구간 페이스 색상 표시 (2026-07-05 M3-B)
- [x] DNF 참가자 경로 표시 (2026-07-05 M3-B)
- [ ] 뷰어 폴리싱 (제품의 얼굴 — 완료 기준을 구체화): 배속 포함 마커 애니메이션 프레임 드랍 없음 / 로딩·GENERATING·FAILED 상태별 UI / 10명 × 600포인트 동시 렌더 성능 확인
- [x] 세션 결과 화면: 순위(동률 표기)·PB 뱃지·DNF/DNS 하단·대기 화면(R-009 정성 문구) — 리플레이 진입은 M3 placeholder (2026-07-05 M2-C)

### FCM 알림 (2종만)
- [x] 세션 리마인더 — 스케줄러+멱등(reminder_notified_at), 발송은 스텁 (Firebase 대기) (2026-07-05 M3-C서버)
  - [x] 리마인더 발송 스케줄러 (clock 주입) (2026-07-05)
- [x] "리플레이가 열렸어요" 트리거 — 최초 READY, 발송은 스텁 (2026-07-05 M3-C서버)
  - [x] 세션당 1회 멱등 (원자적 UPDATE, 재생성 미재발 실증) (2026-07-05)
