...
- [ ] 프로덕션 전환 전 동의항목 검수 요건 확인·정리 (필요 동의항목: 닉네임, 프로필 등)
...
- [ ] 크루 목록·크루 상세 화면 (멤버 목록, 예정 세션)
- [ ] 크루 생성 UI
- [ ] **초대 코드 입력 → 크루 참가 플로우** (클라이언트 + 참가 API) — 코드 "생성·공유"의 수신 측
- [ ] 초대 링크 **딥링크/앱링크 처리**: 카톡에서 링크 탭 → 앱 열림 → 코드 자동 적용
  - [ ] 미설치 시 랜딩 페이지 → Play 비공개 테스트 opt-in 링크 유도 (Cloudflare Pages, 디퍼드 딥링크는 2차)
- [ ] 최초 로그인 닉네임 설정 온보딩
- [ ] 설정/프로필 화면: 닉네임 수정, 로그아웃(자체 토큰·디바이스 토큰 정리), 개인정보처리방침·이용약관 링크
- [ ] **앱 내 회원 탈퇴 UI** — Play 계정 삭제 요건, 없으면 심사 통과 불가 (서버 UserWithdrawn 처리의 진입점)
...
### 인증 + User 컨텍스트
- [ ] 카카오 로그인 연동 (`kakao_flutter_sdk`, Android 키 해시 등록 포함)
- [ ] 앱-서버 토큰 체계: 카카오 토큰 검증 → 자체 토큰(JWT 등) 발급·갱신·만료(401 → 재로그인 플로우) + 서버 인증 필터 — 카카오 회원번호(KakaoAccount)는 인증 어댑터 외부로 노출 금지
- [ ] User 애그리거트: 닉네임, 프로필, 상태(ACTIVE/WITHDRAWN)
- [ ] DeviceToken 관리 (FCM 토큰 등록·기기 교체 시 갱신)
- [ ] 탈퇴 처리(UserWithdrawn 이벤트):
  - [ ] 닉네임 익명화("탈퇴한 러너"), kakao_id·device_token 즉시 파기
  - [ ] track_payload(raw/refined) 지체 없이 삭제
  - [ ] rank_entry·리플레이 파생물은 익명 보존 (크루원들의 리플레이·순위 히스토리 보호)

### Crew 컨텍스트
- [ ] Crew 애그리거트: 이름, 크루장, 멤버 목록, 상태
- [ ] CrewMember: 역할(LEADER/MEMBER), 가입일, 상태
- [ ] InviteCode: 만료 시각 + 사용 횟수 제한, 초대 코드로만 가입 (공개 가입 경로 없음)
- [ ] 초대 링크/코드 생성 + 카톡 공유 플로우
- [ ] 불변식: 크루장 항상 1명 — **크루장 탈퇴 시 가입일 최선임 멤버에게 자동 승계**
- [ ] 마지막 1인 탈퇴 시 크루 CLOSED 처리
- [ ] CrewMemberJoined 이벤트 발행
  - [ ] 합류 "알림" 처리 방식 결정 — 계획서 내부 상충(5.1 "알림 등" vs 6장 "FCM은 2종만"): 인앱 표시로 갈음할지 범위 외로 확정할지
...
- [ ] 코스 등록 ① 지도에서 경로 그리기 (네이버 지도 SDK)
- [ ] 코스 등록 ② 과거 주행 기록을 코스로 승격
- [ ] 불변식: **발행(OPEN 세션에서 사용)된 코스는 불변** — 수정 필요 시 새 코스 생성
...
- [ ] TrackUploadService: 업로드 수신, 폴리라인 디코딩, 기본 검증(코스 이탈 허용 오차, 시간 정합성)
...
- [ ] TrackSegment: 구간(500m 단위) 페이스 요약 — 리플레이 색상용
